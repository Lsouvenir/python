...
gdp1=read_csv_as_nested_dict("isp_gdp.csv","Country Name",",",'"')
#print(gdp1)#对第一个自定义函数进行检测
    ...
    for i in plot_countries.keys():
        for j in gdp_countries:
            if plot_countries[i]==j:
                dict1[i]=plot_countries[i]#将世行有的GDP数据信息以字典的形式输入
            if plot_countries[i]!=j:
                set1.add(i)#此集合的内容为世行没有GDP数据的绘图库国家代码
    tuple1=tuple1+(dict1,)+(set1,)#将上面的内容返回到元组格式
    return(tuple1)
    
#print(reconcile_countries_by_name(pygal_countries,gdp1))  #对第二个自定义函数进行检测
...
def build_map_dict_by_name(gdpinfo, plot_countries, year):
    ...
    for i in plot_countries.keys():
        for j in gdp1:
            ...
            if plot_countries[i]!=j:
                set1.add(i)#该集合为世行没有GDP数据的国家代码
    for i in plot_countries.keys():
        for j in gdp1.values():
            if j[year]!='' and j['Country Name']==plot_countries[i]:
                number=float(j[year])
                year_number=math.log10(number)
                dict2[i]=year_number#字典数据为库中各个国家的代码以及对应的某年的GDP产值，其中键为绘图库国家代码，值为对应的具体国名
            if j[year]=='' and j['Country Name']==plot_countries[i]:
                set2.add(i)#该集合为库中没有某特定年世行数据的国家的代码
...
def render_world_map(gdpinfo, plot_countries, year, map_file): #将具体某年世界各国的GDP数据(包括缺少GDP数据以及只是在该年缺少GDP数据的国家)以地图形式可视化\
    ...
    """
    tup=build_map_dict_by_name(gdpinfo, pygal_countries, year)
    dict1=tup[0]
    list1=[]
    for i in dict1.items():
        list1.append(i)
    worldmap_chart = pygal.maps.world.World()
    worldmap_chart.title = '全球GDP分布图'#输入绘图标题
    worldmap_chart.add(year, list1)#可视化给定年份各国及其在世行对应的gdp值
    worldmap_chart.add('no date at this year',list(tup[2])) #可视化仅该年没有数据的国家
    worldmap_chart.add('missing from word bank',list(tup[1]))#可视化没有数据的国家
